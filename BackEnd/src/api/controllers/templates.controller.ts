...
export async function updateTemplate(req: Request, res: Response) {
  try {
    ...
    if (typeof raw.name === 'string') updatePayload.name = raw.name.trim().slice(0, 200)
    if (typeof raw.role === 'string') updatePayload.role = raw.role.trim().slice(0, 32000)
    if (typeof raw.description === 'string') updatePayload.description = raw.description.trim().slice(0, 800)
    if (typeof raw.icon === 'string' && raw.icon.trim()) updatePayload.icon = raw.icon.trim().slice(0, 64)
    if (typeof raw.complexity === 'string' && raw.complexity.trim()) {
      const c = raw.complexity.trim()
      if (['Simple', 'Intermediate', 'Advanced'].includes(c)) updatePayload.complexity = c
    }
