...
export function detectJailbreak(message: string): boolean {
  ...
  for (const pattern of jailbreakPatterns) {
    if (pattern.test(lowerMessage)) {
      logger.warn('[detectJailbreak] 🛡️ Tentativa de jailbreak detectada:', {
 ...
 */
export function detectCompetitorMention(message: string): boolean {
  if (!message || !message.trim()) return false
  ...
  for (const keyword of competitorKeywords) {
    if (lowerMessage.includes(keyword)) {
      logger.warn('[detectCompetitorMention] 🛡️ Menção a concorrente detectada:', {
        keyword,
        messagePreview: message.substring(0, 100)
      })
      return true
  ...
  for (const pattern of competitorPatterns) {
    if (pattern.test(lowerMessage)) {
      logger.warn('[detectCompetitorMention] 🛡️ Padrão de concorrente detectado:', {
        pattern: pattern.toString(),
        messagePreview: message.substring(0, 100)
      })
      return true
