...
export function validateMetaWhatsappFlowPayload(
  nodesJson: unknown
  ...
  for (const n of nodes) {
    ...
    if (n.type === 'wa_template') {
      ...
      if (!name) {
        warnings.push(`Bloco Template WhatsApp (node ${id}): nome do template Meta vazio.`)
      }

      if (!lang) {
        warnings.push(`Bloco Template WhatsApp (node ${id}): idioma vazio.`)
      }
    }
    ...
    if (n.type === 'whatsapp_message') {
      ...
      if (!text) {
        warnings.push(`Bloco Mensagem WhatsApp (node ${id}): texto da mensagem vazio.`)
      }

      if (String(d.waMessageType || '').trim() === 'buttons') {
        const buttons = Array.isArray(d.waButtons) ? d.waButtons : []
        if (buttons.length === 0) {
          warnings.push(`Bloco Mensagem WhatsApp (node ${id}): adicione ao menos um botão.`)
        }
      }

      if (String(d.waWindowMode || 'session_only').trim() !== 'auto_template') {
        warnings.push(
          `Bloco Mensagem WhatsApp (node ${id}): use de preferência após a validação da Janela 24h.`
        )
      }
    }
  }
  ...
  if (nodes.some((n) => n && typeof n === 'object' && n.type === 'wa_session_window')) {
    warnings.push(
      'Fluxo com bloco Janela 24h: confirme os ramos dentro/fora; fora da janela costuma exigir Template WhatsApp.'
    )
  }
    ...
    for (const n of nodes) {
      ...
      if (n.type === 'wa_template') {
        ...
        if (!String(d.waTemplateName || '').trim()) {
          errors.push(`Node ${id}: waTemplateName obrigatório em modo estrito.`)
        }
        if (!String(d.waTemplateLanguage || '').trim()) {
          errors.push(`Node ${id}: waTemplateLanguage obrigatório em modo estrito.`)
        }
      }

      if (n.type === 'whatsapp_message' && !String(n.data?.waMessageText || '').trim()) {
        errors.push(`Node ${id}: waMessageText obrigatório em modo estrito.`)
      }
    }
  }
