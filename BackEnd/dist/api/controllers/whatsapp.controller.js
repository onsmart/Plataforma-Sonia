...
function maskWhatsAppPhoneNumber(phone) {
...
function maskWhatsAppAppKey(appKey) {
...
function normalizeMetaCallStartedAt(timestamp) {
...
function getRequestAuthEmail(req) {
    return String(req.user?.email || '').trim() || null;
}
function respondWhatsAppOwnershipError(res, error) {
...
async function processInboundWhatsAppCall(metaCall) {
    ...
    if (!integration) {
        logger_1.default.warn('[receiveWhatsAppWebhook] Integracao Meta nao encontrada para chamada recebida', {
    ...
    logger_1.default.info('[receiveWhatsAppWebhook] Chamada Meta recebida para processamento', {
    ...
    try {
        ...
        logger_1.default.error('[receiveWhatsAppWebhook] Erro ao buscar agente vinculado para chamada WhatsApp', {
            integrationId: integration.id,
            callId: metaCall.callId,
    ...
    const rejectCall = async (reason) => {
        logger_1.default.warn('[receiveWhatsAppWebhook] Chamada WhatsApp sera recusada', {
            integrationId: integration.id,
    ...
    let voiceProfile;
    try {
        voiceProfile = await (0, voiceProfile_service_1.getStoredAgentVoiceProfile)(agent.id);
    }
    catch (error) {
        logger_1.default.error('[receiveWhatsAppWebhook] Erro ao buscar perfil de voz para chamada WhatsApp', {
            integrationId: integration.id,
            agentId: agent.id,
            callId: metaCall.callId,
            error: error?.message
    ...
    if (!voiceProfile?.enabled || !voiceProfile.voiceId) {
        logger_1.default.warn('[receiveWhatsAppWebhook] Perfil de voz do agente indisponivel para chamada', {
            integrationId: integration.id,
            agentId: agent.id,
            callId: metaCall.callId,
    ...
    if (!supportsRealtime || !realtimeService.prepareInboundWhatsAppCall) {
        logger_1.default.warn('[receiveWhatsAppWebhook] Chamada habilitada, mas runtime realtime ainda nao esta registrado', {
            integrationId: integration.id,
            agentId: agent.id,
            callId: metaCall.callId
        });
    ...
    const { data: integrationWithUserForCall, error: integrationUserForCallError } = await supabase_1.supabase
        .from('tb_integrations')
        .select(`
      user_id,
    ...
    if (integrationUserForCallError) {
        logger_1.default.warn('[receiveWhatsAppWebhook] Falha ao buscar email do dono para chamada WhatsApp', {
            integrationId: integration.id,
            callId: metaCall.callId,
    ...
    logger_1.default.info('[receiveWhatsAppWebhook] SDP answer preparado para chamada WhatsApp', {
        integrationId: integration.id,
        agentId: agent.id,
        callId: metaCall.callId,
        phoneNumberId: metaCall.phoneNumberId || null,
    ...
    if (!preAcceptResult.success) {
        logger_1.default.warn('[receiveWhatsAppWebhook] Falha no pre_accept da chamada WhatsApp', {
            integrationId: integration.id,
            agentId: agent.id,
            callId: metaCall.callId,
            phoneNumberId: metaCall.phoneNumberId || null,
    ...
    if (!acceptResult.success) {
        logger_1.default.warn('[receiveWhatsAppWebhook] Falha no accept da chamada WhatsApp', {
            integrationId: integration.id,
            agentId: agent.id,
            callId: metaCall.callId,
            phoneNumberId: metaCall.phoneNumberId || null,
...
async function getWhatsAppStatus(req, res) {
    try {
        ...
        if (respondWhatsAppOwnershipError(res, error))
            return;
...
async function receiveWhatsAppWebhook(req, res) {
    try {
        ...
        const extractedCalls = (0, whatsapp_meta_1.extractMetaWebhookCalls)(webhookData);
        ...
        if (extractedCalls.length === 0 && extractedMessages.length === 0 && extractedStatuses.length === 0) {
            logger_1.default.log('[receiveWhatsAppWebhook] Evento oficial da Meta recebido sem chamadas, mensagens ou status processaveis');
            return res.status(200).json({
                received: true,
                ignored: true,
        ...
        for (const metaCall of extractedCalls) {
            try {
                ...
                logger_1.default.error('[receiveWhatsAppWebhook] Erro ao processar chamada recebida da Meta', {
                    callId: metaCall.callId,
                    phoneNumberId: metaCall.phoneNumberId,
...
async function deleteWhatsAppConversationHistory(req, res) {
    try {
        if (!req.user?.email) {
            return res.status(401).json({ error: 'Usuario nao autenticado' });
        }
        ...
        const { error: eventsError, count: eventsCount } = await supabase_1.supabase
        ...
        const { error: decisionsError, count: decisionsCount } = await supabase_1.supabase
        ...
        const { error: messagesError, count: messagesCount } = await supabase_1.supabase
            .from('tb_whatsapp_messages')
            .delete({ count: 'exact' })
            .eq('integrations_id', integrationId)
            .eq('whatsapp_contact_id', contactId);
        ...
        const redisReferences = Array.from(new Set([contactData?.phone_number, contactData?.lid]
            .map((value) => String(value || '').trim())
            .filter(Boolean)));
        for (const reference of redisReferences) {
            const clearResult = await (0, whatsapp_redis_1.clearHistory)(integrationId, reference);
        ...
        logger_1.default.error('[deleteWhatsAppConversationHistory] Erro ao apagar historico da conversa', {
