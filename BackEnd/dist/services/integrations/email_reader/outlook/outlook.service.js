...
const outlook_client_1 = require("./outlook.client");
async function readOutlookEmails(integrationsId, limit = 5) {
    ...
    const client = new outlook_client_1.OutlookClient(data.access_token);
    const result = await client.getInboxMessages(limit);
    return result.value.map((email) => ({
        id: email.id,
