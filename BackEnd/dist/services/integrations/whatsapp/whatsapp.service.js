...
exports.fetchMessagesFromEvolutionAPI = fetchMessagesFromEvolutionAPI;
...
exports.fetchChatsFromEvolutionAPI = fetchChatsFromEvolutionAPI;
exports.fetchMessagesFromChat = fetchMessagesFromChat;
 ...
 */
async function resolveConversationId(remoteJid, payload, instanceName, apiUrl, apiKey) {
    try {
        // Se já é um número real (@s.whatsapp.net), retorna direto
        if (remoteJid.endsWith('@s.whatsapp.net')) {
            logger_1.default.log('[resolveConversationId] ✅ Número real detectado:', {
                remoteJid,
                source: 'direct'
            });
            return {
        ...
        if (remoteJid.endsWith('@lid')) {
            logger_1.default.log('[resolveConversationId] 🔍 LID detectado, tentando resolver:', {
                remoteJid
            });
            // 1. Tenta buscar no cache Redis primeiro
            try {
                const { getRedisClient } = await Promise.resolve().then(() => __importStar(require('../../../lib/redis')));
                const redis = await getRedisClient();
                const cacheKey = `whatsapp:lid:${instanceName}:${remoteJid}`;
                const cached = await redis.get(cacheKey);
                if (cached) {
                    logger_1.default.log('[resolveConversationId] ✅ Número real encontrado no cache:', {
                        lid: remoteJid,
                        realNumber: cached,
                        source: 'cache'
                    });
                ...
                logger_1.default.warn('[resolveConversationId] ⚠️ Erro ao buscar cache:', {
                ...
                if (payload.data?.chats) {
                    ...
                    for (const chat of chats) {
                        if (chat.id === remoteJid || chat.jid === remoteJid) {
                            ...
                            if (realJid) {
                                logger_1.default.log('[resolveConversationId] ✅ Número real encontrado no webhook (chats):', {
                                    lid: remoteJid,
                                    realNumber: realJid,
                                    source: 'webhook'
                                });
                ...
                if (payload.data?.contacts) {
                    ...
                    for (const contact of contacts) {
                        if (contact.id === remoteJid || contact.jid === remoteJid) {
                            ...
                            if (realJid) {
                                logger_1.default.log('[resolveConversationId] ✅ Número real encontrado no webhook (contacts):', {
                                    lid: remoteJid,
                                    realNumber: realJid,
                                    source: 'webhook'
                                });
            ...
            try {
                ...
                try {
                    const response = await axios_1.default.get(`${apiUrl}/contact/fetchStatus/${instanceName}`, {
                        headers: {
                            'apikey': apiKey,
                    ...
                    if (response.data?.number) {
                        ...
                        if (number.length >= 10 && number.length <= 15 && !number.startsWith('14') && !number.startsWith('17')) {
                            ...
                            logger_1.default.log('[resolveConversationId] ✅ Número real encontrado na API:', {
                                lid: remoteJid,
                                realNumber: realJid,
                                source: 'api'
                            });
                ...
                try {
                    ...
                    if (chatResponse.data && Array.isArray(chatResponse.data)) {
                        ...
                        if (chat?.remoteJid?.endsWith('@s.whatsapp.net')) {
                            logger_1.default.log('[resolveConversationId] ✅ Número real encontrado nos chats:', {
                                lid: remoteJid,
                ...
                logger_1.default.warn('[resolveConversationId] ⚠️ Erro ao buscar na API:', {
                    error: apiError?.message
                });
            }
            // 4. Se não conseguiu resolver, retorna o LID original
            logger_1.default.warn('[resolveConversationId] ⚠️ Não foi possível resolver LID, usando original:', {
                lid: remoteJid
            });
            return {
                originalId: remoteJid,
        ...
        if (cleanNumber.length >= 10 && cleanNumber.length <= 15) {
            ...
            logger_1.default.log('[resolveConversationId] ✅ Número sem sufixo, normalizado:', {
                original: remoteJid,
                normalized: realJid,
                source: 'direct'
            });
        ...
        logger_1.default.error('[resolveConversationId] ❌ Erro ao resolver ID:', {
            remoteJid,
            error: error?.message
        });
        return {
 ...
 */
async function saveLidMapping(instanceName, lid, realJid) {
    try {
        const { getRedisClient } = await Promise.resolve().then(() => __importStar(require('../../../lib/redis')));
        const redis = await getRedisClient();
        ...
        logger_1.default.log('[saveLidMapping] ✅ Mapeamento salvo no cache:', {
            lid,
            realJid
        });
    }
    catch (error) {
        logger_1.default.warn('[saveLidMapping] ⚠️ Erro ao salvar cache:', {
            error: error?.message
        });
    }
}
 ...
 */
async function getEvolutionAPICredentials(integrationsId) {
    try {
        ...
        logger_1.default.log('[getEvolutionAPICredentials] Configurações:', {
            apiUrl,
        ...
        const { data, error } = await supabase_1.supabase
            ...
            .single();
        if (error || !data) {
            logger_1.default.error('[getEvolutionAPICredentials] Erro ao buscar integração:', {
                integrationsId,
        ...
        if (!instanceName) {
            logger_1.default.error('[getEvolutionAPICredentials] instanceName não encontrado:', {
                integrationsId,
        ...
        logger_1.default.error('[getEvolutionAPICredentials] Erro:', {
 ...
 */
async function checkEvolutionAPIHealth(config) {
    try {
        logger_1.default.log('[checkEvolutionAPIHealth] Verificando saúde da EvolutionAPI...', {
        ...
        logger_1.default.log('[checkEvolutionAPIHealth] ✅ EvolutionAPI está acessível');
    }
    catch (error) {
        const errorDetails = {
        ...
        logger_1.default.error('[checkEvolutionAPIHealth] ❌ EvolutionAPI não está acessível:', errorDetails);
        if (error?.code === 'ECONNREFUSED') {
            throw new Error(`❌ EvolutionAPI não está rodando ou não está acessível em ${config.apiUrl}.\n\n` +
                `📋 Verifique:\n` +
 ...
 */
async function createOrGetInstance(config) {
    try {
        logger_1.default.log('[createOrGetInstance] Verificando se instância existe...', {
            instanceName: config.instanceName,
            apiUrl: config.apiUrl
        });
        // Verifica se a instância já existe
        try {
            const checkResponse = await axios_1.default.get(`${config.apiUrl}/instance/fetchInstances`, {
                headers: {
                    'apikey': config.apiKey
                },
            ...
            if (existingInstance) {
                const state = existingInstance.instance?.state ||
                    ...
                    'unknown';
                logger_1.default.log(`[createOrGetInstance] ✅ Instância ${config.instanceName} já existe`, {
                    state: state
                });
            ...
            logger_1.default.warn('[createOrGetInstance] ⚠️ Erro ao verificar instâncias existentes, tentando criar...', {
                error: checkError?.message
            });
        }
        // Cria nova instância apenas se não existir
        logger_1.default.log(`[createOrGetInstance] Criando nova instância: ${config.instanceName}`);
        try {
            const createResponse = await axios_1.default.post(`${config.apiUrl}/instance/create`, {
                instanceName: config.instanceName,
                token: config.apiKey,
                qrcode: true,
            ...
            logger_1.default.log(`[createOrGetInstance] ✅ Instância ${config.instanceName} criada com sucesso`, {
            ...
            const instanceAlreadyExists = status === 403 ||
                status === 400 ||
                status === 409 ||
                (typeof errorMessage === 'string' && (errorMessage.toLowerCase().includes('already in use') ||
                    errorMessage.toLowerCase().includes('already exists') ||
                    errorMessage.toLowerCase().includes('duplicate') ||
                    errorMessage.toLowerCase().includes('forbidden'))) ||
                (Array.isArray(errorMessage) && errorMessage.some((msg) => msg.toLowerCase().includes('already in use') ||
                    msg.toLowerCase().includes('already exists')));
            if (instanceAlreadyExists) {
                logger_1.default.log(`[createOrGetInstance] ✅ Instância ${config.instanceName} já existe (erro ${status}), usando a existente`, {
        ...
        logger_1.default.error('[createOrGetInstance] ❌ Erro detalhado:', errorDetails);
        logger_1.default.error('[createOrGetInstance] ❌ Erro completo:', error);
        ...
        if (error?.response?.status === 403 ||
            ...
            logger_1.default.warn('[createOrGetInstance] ⚠️ Erro indica que instância pode já existir, usando a existente...');
            return config.instanceName;
        }
        throw new Error(`Erro ao criar/obter instância: ${errorDetails.message}${errorDetails.response ? ` - ${JSON.stringify(errorDetails.response)}` : ''}`);
    }
}
/**
...
async function checkConnectionStatus(integrationsId) {
    try {
        ...
        const response = await axios_1.default.get(`${config.apiUrl}/instance/fetchInstances`, {
            headers: {
                'apikey': config.apiKey
            },
            timeout: 10000
        ...
        if (!instance) {
            logger_1.default.warn('[checkConnectionStatus] Instância não encontrada:', {
                instanceName: config.instanceName,
        ...
        logger_1.default.log('[checkConnectionStatus] Status da instância:', {
            instanceName: config.instanceName,
            state: state,
        ...
        logger_1.default.error('[checkConnectionStatus] ❌ Erro:', errorDetails);
        return 'disconnected';
    }
}
/**
...
async function getQRCode(integrationsId) {
    try {
        ...
        if (status === 'connected') {
            logger_1.default.log('[getQRCode] ✅ Instância já está conectada (status: connected/open), QR Code não necessário');
        ...
        try {
            ...
            const instances = checkResponse.data || [];
            const existingInstance = instances.find((inst) => (inst.instance?.instanceName || inst.instanceName) === config.instanceName);
            if (!existingInstance) {
                // Só cria se não existir
                logger_1.default.log('[getQRCode] Instância não existe, criando...');
                await createOrGetInstance(config);
            }
            else {
                logger_1.default.log('[getQRCode] Instância já existe, pulando criação');
            }
        }
        catch (error) {
            // Se der erro ao verificar, tenta criar mesmo assim
            logger_1.default.warn('[getQRCode] Erro ao verificar instância, tentando criar...', error.message);
            try {
                await createOrGetInstance(config);
            }
            catch (createError) {
                ...
                if (createError?.response?.status === 403) {
                    logger_1.default.log('[getQRCode] Instância já existe (erro 403), continuando...');
                }
                else {
                    throw createError;
                }
        ...
        // Busca o QR Code
        logger_1.default.log('[getQRCode] Buscando QR Code...', {
            instanceName: config.instanceName,
            apiUrl: config.apiUrl
        });
        ...
        const response = await axios_1.default.get(`${config.apiUrl}/instance/connect/${config.instanceName}`, {
            headers: {
                'apikey': config.apiKey
            },
            timeout: 10000
        ...
        logger_1.default.log('[getQRCode] Resposta da Evolution API:', {
        ...
        if (responseState === 'open' || responseState === 'connected') {
            logger_1.default.log(`[getQRCode] ✅ Instância está ${responseState} (conectada), QR Code não necessário`);
        ...
        if (qrCode) {
            ...
            console.log('═══════════════════════════════════════════════════════════════');
            console.log('');
            logger_1.default.log('[getQRCode] ✅ QR Code gerado e exibido com sucesso');
            return { qrCode: qrCode, isConnected: false };
        }
        else {
            logger_1.default.warn('[getQRCode] ⚠️ QR Code não encontrado na resposta:', {
        ...
        logger_1.default.error('[getQRCode] ❌ Erro ao obter QR Code:', errorDetails);
        logger_1.default.error('[getQRCode] ❌ Erro completo:', error);
        return { qrCode: null, isConnected: false };
    }
}
/**
...
async function sendWhatsApp(integrationsId, data) {
    try {
        ...
        try {
            ...
            logger_1.default.error('[sendWhatsApp] ❌ EvolutionAPI não está acessível:', {
        ...
        try {
            ...
            logger_1.default.error('[sendWhatsApp] ❌ Erro ao criar/obter instância:', {
        ...
        if (status !== 'connected') {
            logger_1.default.warn(`[sendWhatsApp] ⚠️ Instância não está conectada. Status: ${status}`);
            ...
            try {
                ...
                if (qrCodeResult.isConnected) {
                    logger_1.default.log('[sendWhatsApp] ✅ Instância está conectada (detectado por getQRCode), prosseguindo com envio');
                    ...
                    logger_1.default.log('[sendWhatsApp] ✅ Instância conectou após obter QR Code, prosseguindo com envio...');
                }
                else {
                ...
                logger_1.default.error('[sendWhatsApp] ⚠️ Erro ao obter QR Code:', {
                ...
                logger_1.default.log('[sendWhatsApp] ✅ Instância conectou, prosseguindo com envio...');
            }
        }
        else {
            logger_1.default.log('[sendWhatsApp] ✅ Instância está conectada, prosseguindo com envio...');
        }
        ...
        logger_1.default.log('[sendWhatsApp] 📚 Buscando histórico do Redis...', {
            conversationId: data.to,
            integrationsId
        });
        const { getHistoryFromRedis } = await Promise.resolve().then(() => __importStar(require('./whatsapp.redis')));
        const history = await getHistoryFromRedis(integrationsId, data.to, // Usa ID da conversa completo
        10 // últimas 10 mensagens
        );
        logger_1.default.log('[sendWhatsApp] Histórico encontrado no Redis:', {
            count: history.length
        });
        // 6️⃣ Envia a mensagem (somente se conectado)
        logger_1.default.log('[sendWhatsApp] 📤 Enviando mensagem...', {
            instanceName,
            to: data.to,
        ...
        if (!data.to || data.to.trim() === '') {
            logger_1.default.error('[sendWhatsApp] ❌ data.to está vazio ou undefined:', {
                data: data
            });
            return {
                success: false,
        ...
        logger_1.default.log('[sendWhatsApp] 📱 ID da conversa recebido (usando exatamente como veio):', {
        ...
        if (contactNumberResult.success && contactNumberResult.number) {
            ...
            console.log('='.repeat(80) + '\n');
            logger_1.default.log('[sendWhatsApp] ✅ Número do contato encontrado no banco:', {
            ...
            console.log('='.repeat(80) + '\n');
            logger_1.default.error('[sendWhatsApp] ❌ Erro ao buscar número do contato no banco:', {
                contactId: originalConversationId,
        ...
        const { getRealNumberFromLid } = await Promise.resolve().then(() => __importStar(require('./whatsapp.redis')));
        if (conversationIdForSending.endsWith('@lid')) {
            ...
            if (realNumber) {
                ...
                console.log('='.repeat(80) + '\n');
                logger_1.default.log('[sendWhatsApp] ✅ Número real encontrado no Redis, usando para envio:', {
                ...
                const resolved = await resolveConversationId(conversationIdForSending, null, instanceName, config.apiUrl, config.apiKey);
        ...
        const resolved = await resolveConversationId(originalConversationId, null, // Não temos payload do webhook aqui
        instanceName, config.apiUrl, config.apiKey);
        ...
        logger_1.default.log('[sendWhatsApp] 📤 Preparando envio:', {
        ...
        if (isLid) {
            logger_1.default.log('[sendWhatsApp] 📤 Enviando mensagem para @lid (suportado pela Evolution API):', {
                lid: conversationIdForSending,
                messageLength: data.message.length
            });
            ...
            logger_1.default.warn('[sendWhatsApp] ⚠️ ID com formato desconhecido:', {
        ...
        if (isLid) {
            ...
            logger_1.default.log('[sendWhatsApp] 📤 Enviando para @lid usando campo "phone":', {
            ...
            logger_1.default.log('[sendWhatsApp] 📤 Enviando com remoteJid + number:', {
            ...
            const cleanNumber = conversationIdForSending.replace(/@.*$/, '').replace(/\D/g, '');
            payload.number = cleanNumber;
            logger_1.default.log('[sendWhatsApp] 📤 Enviando com number (sem sufixo):', {
                number: cleanNumber,
        ...
        let response;
        try {
            response = await axios_1.default.post(`${config.apiUrl}/message/sendText/${instanceName}`, payload, {
                headers: {
                    'apikey': config.apiKey,
                    'Content-Type': 'application/json'
                },
            ...
            logger_1.default.error('[sendWhatsApp] ❌ Erro ao enviar mensagem:', {
            ...
            if (error.code === 'ECONNABORTED' || error.message?.includes('timeout')) {
                logger_1.default.error('[sendWhatsApp] ❌ Timeout ao enviar mensagem:', {
                    conversationIdForSending,
                    timeout: '30s',
                    apiUrl: config.apiUrl
                });
            ...
            const isContactNotFound = Array.isArray(errorResponseData) &&
                errorResponseData.some((msg) => msg.exists === false);
            if (isContactNotFound || (error?.response?.status === 400 && isLid)) {
                logger_1.default.warn('[sendWhatsApp] ⚠️ Contato não encontrado ou @lid não suportado, salvando na fila:', {
                ...
                try {
                    const { enqueueResponse } = await Promise.resolve().then(() => __importStar(require('./whatsapp.queue')));
            ...
            `)
                    ...
                    if (integration && agent) {
                        const userEmail = integration.tb_users?.email;
                        if (userEmail) {
                            ...
                            if (queueResult.success) {
                                logger_1.default.log('[sendWhatsApp] ✅ Mensagem salva na fila (contato não encontrado):', {
                    ...
                    logger_1.default.error('[sendWhatsApp] ⚠️ Erro ao adicionar à fila:', {
            ...
            if (isLid && error?.response?.status === 400 && payload.phone) {
                logger_1.default.warn('[sendWhatsApp] ⚠️ Bad Request com formato "phone" + @lid, tentando outros formatos...');
                ...
                try {
                    ...
                    if (cleanNumber.length >= 10 && cleanNumber.length <= 15) {
                        ...
                        console.log('='.repeat(80) + '\n');
                        logger_1.default.log('[sendWhatsApp] 🔄 Tentativa 1: remoteJid + number:', {
                            remoteJid: conversationIdForSending,
                            number: cleanNumber
                        });
                        response = await axios_1.default.post(`${config.apiUrl}/message/sendText/${instanceName}`, retryPayload1, {
                            headers: {
                                'apikey': config.apiKey,
                                'Content-Type': 'application/json'
                            },
                        ...
                        console.log('='.repeat(80) + '\n');
                        logger_1.default.log('[sendWhatsApp] ✅ Sucesso na tentativa 1 (remoteJid + number)');
                    }
                }
                catch (retryError1) {
                    logger_1.default.warn('[sendWhatsApp] ⚠️ Tentativa 1 falhou, tentando formato @s.whatsapp.net...');
                    ...
                    try {
                        ...
                        if (cleanNumber.length >= 10 && cleanNumber.length <= 15) {
                            ...
                            logger_1.default.log('[sendWhatsApp] 🔄 Tentativa 2: número extraído com @s.whatsapp.net:', {
                                remoteJid: retryRemoteJid,
                                number: cleanNumber
                            });
                            console.log('\n' + '='.repeat(80));
                            ...
                            console.log('='.repeat(80) + '\n');
                            response = await axios_1.default.post(`${config.apiUrl}/message/sendText/${instanceName}`, retryPayload2, {
                                headers: {
                                    'apikey': config.apiKey,
                                    'Content-Type': 'application/json'
                                },
                            ...
                            console.log('Data:', JSON.stringify(response.data, null, 2));
                            console.log('='.repeat(80) + '\n');
                            logger_1.default.log('[sendWhatsApp] ✅ Sucesso na tentativa 2 (@s.whatsapp.net)');
                        }
                        else {
                        ...
                        console.error('='.repeat(80) + '\n');
                        logger_1.default.error('[sendWhatsApp] ❌ Todas as tentativas falharam, salvando na fila:', {
                            conversationIdForSending,
                ...
                logger_1.default.warn('[sendWhatsApp] ⚠️ Bad Request com number, tentando com remoteJid...');
                try {
                    const cleanNumber = conversationIdForSending.replace(/@.*$/, '').replace(/\D/g, '');
                    const retryRemoteJid = `${cleanNumber}@s.whatsapp.net`;
                    const retryPayload = {
                    ...
                    logger_1.default.log('[sendWhatsApp] 🔄 Tentando retry com remoteJid:', {
                        original: conversationIdForSending,
                        retryRemoteJid
                    });
                    response = await axios_1.default.post(`${config.apiUrl}/message/sendText/${instanceName}`, retryPayload, {
                        headers: {
                            'apikey': config.apiKey,
                            'Content-Type': 'application/json'
                        },
                    ...
                    logger_1.default.log('[sendWhatsApp] ✅ Sucesso ao tentar com remoteJid:', {
                        remoteJid: retryRemoteJid
                    });
                }
                catch (retryError) {
                    logger_1.default.error('[sendWhatsApp] ❌ Erro também ao tentar com remoteJid:', {
                        conversationIdForSending,
        ...
        logger_1.default.log(`[sendWhatsApp] ✅ Mensagem enviada com sucesso para ${conversationIdForSending}`, {
            messageId,
        ...
        logger_1.default.log('[sendWhatsApp] 💾 ID para salvar no banco/Redis:', {
        ...
        try {
            const { saveMessageToHistory } = await Promise.resolve().then(() => __importStar(require('./whatsapp.redis')));
            await saveMessageToHistory(integrationsId, conversationIdForDb, // Usa número real quando disponível
            'assistant', data.message);
            logger_1.default.log('[sendWhatsApp] ✅ Mensagem salva no Redis');
        }
        catch (saveError) {
            // Não bloqueia o envio se falhar ao salvar
            logger_1.default.error('[sendWhatsApp] ⚠️ Erro ao salvar mensagem no Redis:', {
                error: saveError?.message
            });
        }
        // 8️⃣ Busca ou cria contato e salva a mensagem enviada no banco de dados
        let contactId = null; // Declarado fora do try para ser usado depois
        try {
            // Busca contato pelo LID ou número real
            const { getContactByLid, getContactByPhoneNumber, createOrUpdateContact } = await Promise.resolve().then(() => __importStar(require('./whatsapp.contacts')));
            ...
            if (contactId) {
                ...
                if (dbResult.success) {
                    ...
                    logger_1.default.error('[sendWhatsApp] ⚠️ Erro ao salvar no banco:', {
                ...
                logger_1.default.warn('[sendWhatsApp] ⚠️ Não foi possível obter contactId, mensagem não será salva no banco:', {
                    conversationId: conversationIdForSending
                });
            }
        }
            ...
            // Não bloqueia o envio se falhar ao salvar
            logger_1.default.error('[sendWhatsApp] ⚠️ Erro ao salvar mensagem no banco (não bloqueia envio):', {
        ...
        try {
            ...
            let contactIdForMarking = null;
            // Se já temos o contactId da mensagem salva, usa ele
            if (contactId) {
                contactIdForMarking = contactId;
                logger_1.default.log('[sendWhatsApp] ✅ Usando contactId da mensagem salva para marcar como lida:', {
                    contactId: contactIdForMarking
                });
            }
            else {
                // Se não temos, tenta buscar pelo conversationId
                logger_1.default.log('[sendWhatsApp] 🔍 Buscando contactId para marcar mensagens como lidas...', {
                    conversationId: conversationIdForDb
                });
                // Tenta buscar contato pelo LID ou número
                const { getContactByLid, getContactByPhoneNumber } = await Promise.resolve().then(() => __importStar(require('./whatsapp.contacts')));
                ...
                if (contactIdForMarking) {
                    logger_1.default.log('[sendWhatsApp] ✅ ContactId encontrado para marcar mensagens como lidas:', {
                        contactId: contactIdForMarking,
                        conversationId: conversationIdForDb
                    });
                }
                else {
                    logger_1.default.warn('[sendWhatsApp] ⚠️ Não foi possível encontrar contactId para marcar mensagens como lidas:', {
                        conversationId: conversationIdForDb
                    });
                }
            }
            ...
            if (contactIdForMarking) {
                ...
                if (markResult.success) {
                    logger_1.default.log('[sendWhatsApp] ✅ Mensagens não lidas marcadas como lidas:', {
                        contactId: contactIdForMarking,
                        conversationId: conversationIdForDb
                    });
                    ...
                    logger_1.default.error('[sendWhatsApp] ⚠️ Erro ao marcar mensagens como lidas:', {
                ...
                logger_1.default.warn('[sendWhatsApp] ⚠️ Não foi possível marcar mensagens como lidas: contactId não encontrado', {
                    conversationId: conversationIdForDb
                });
            }
        }
            ...
            logger_1.default.error('[sendWhatsApp] ⚠️ Erro ao marcar mensagens como lidas (não bloqueia envio):', {
        ...
        logger_1.default.error('[sendWhatsApp] ❌ Erro detalhado ao enviar mensagem:', errorDetails);
        logger_1.default.error('[sendWhatsApp] ❌ Erro completo:', error);
        ...
        try {
            ...
            if (isConnected) {
                logger_1.default.log('[sendWhatsApp] ✅ Instância está conectada, tentando enviar novamente...');
                // Pode tentar enviar novamente ou retornar sucesso
            }
        }
        catch (qrError) {
            logger_1.default.error('[sendWhatsApp] ⚠️ Erro ao obter QR Code após falha:', {
                error: qrError.message
            });
        }
 ...
 */
async function fetchMessagesFromEvolutionAPI(integrationsId, limit = 50) {
    try {
        const config = await getEvolutionAPICredentials(integrationsId);
        if (!config) {
        ...
        logger_1.default.log('[fetchMessagesFromEvolutionAPI] Buscando mensagens da Evolution API...', {
            instanceName,
            limit
        });
        ...
        const response = await axios_1.default.get(`${config.apiUrl}/chat/fetchMessages/${instanceName}`, {
            headers: {
                'apikey': config.apiKey,
                'Content-Type': 'application/json'
            },
            ...
            logger_1.default.warn('[fetchMessagesFromEvolutionAPI] Endpoint fetchMessages não disponível, tentando chat...');
            // Tenta buscar via endpoint de chat (se disponível)
            try {
                const chatResponse = await axios_1.default.get(`${config.apiUrl}/chat/fetchChats/${instanceName}`, {
                    headers: {
                        'apikey': config.apiKey,
                        'Content-Type': 'application/json'
                    },
                ...
                logger_1.default.error('[fetchMessagesFromEvolutionAPI] ❌ Erro ao buscar mensagens:', {
        ...
        logger_1.default.log('[fetchMessagesFromEvolutionAPI] ✅ Mensagens buscadas:', {
        ...
        logger_1.default.error('[fetchMessagesFromEvolutionAPI] ❌ Erro:', {
            message: error?.message,
...
async function saveWhatsAppMessage(data) {
    try {
        logger_1.default.log('[saveWhatsAppMessage] Salvando mensagem...', {
        ...
        logger_1.default.log('[saveWhatsAppMessage] Definindo is_read:', {
            direction: data.direction,
            is_read: isRead
        });
        ...
        logger_1.default.log('[saveWhatsAppMessage] ✅ Mensagem salva com sucesso', {
            id: savedData?.id,
            direction: data.direction,
            is_read: isRead,
            whatsapp_contact_id: data.whatsapp_contact_id
        ...
        logger_1.default.error('[saveWhatsAppMessage] ❌ Erro:', {
            message: error?.message,
            stack: error?.stack
        });
        return {
...
async function getWhatsAppHistory(contactIdOrLid, integrationsId, limit = 10, agentId, sinceTimestamp) {
    try {
        logger_1.default.log('[getWhatsAppHistory] Buscando histórico...', {
            contactIdOrLid,
            integrations_id: integrationsId,
            limit,
            agent_id: agentId,
        ...
        if (contactIdOrLid.endsWith('@lid') || !contactIdOrLid.includes('-')) {
            ...
            const { getContactByLid } = await Promise.resolve().then(() => __importStar(require('./whatsapp.contacts')));
        ...
        if (error) {
            logger_1.default.error('[getWhatsAppHistory] ❌ Erro ao buscar histórico:', {
                error: error.message
            });
            return [];
        }
        logger_1.default.log('[getWhatsAppHistory] ✅ Histórico encontrado', {
            count: data?.length || 0
        });
        return (data || []);
    }
    catch (error) {
        logger_1.default.error('[getWhatsAppHistory] ❌ Erro:', {
            message: error?.message,
            stack: error?.stack
        });
        return [];
 ...
 */
async function getUnreadMessages(contactIdOrLid, integrationsId, sinceTimestamp, agentId) {
    try {
        logger_1.default.log('[getUnreadMessages] Buscando mensagens não lidas...', {
            contactIdOrLid,
            integrations_id: integrationsId,
            since_timestamp: sinceTimestamp,
            agent_id: agentId
        ...
        const { data, error } = await query;
        if (error) {
            logger_1.default.error('[getUnreadMessages] ❌ Erro:', {
                error: error.message
            });
            return [];
        }
        logger_1.default.log('[getUnreadMessages] ✅ Mensagens não lidas encontradas', {
            count: data?.length || 0
        });
        return (data || []);
    }
 ...
 */
async function fetchChatsFromEvolutionAPI(integrationsId) {
    try {
        const config = await getEvolutionAPICredentials(integrationsId);
        if (!config) {
            return { success: false, error: 'Credenciais não encontradas' };
        ...
        const instanceName = await createOrGetInstance(config);
        logger_1.default.log('[fetchChatsFromEvolutionAPI] Buscando chats da Evolution API...', {
            instanceName
        });
        // Evolution API endpoint: GET /chat/fetchChats/{instanceName}
        const response = await axios_1.default.get(`${config.apiUrl}/chat/fetchChats/${instanceName}`, {
            headers: {
                'apikey': config.apiKey,
                'Content-Type': 'application/json'
            },
        ...
        logger_1.default.log('[fetchChatsFromEvolutionAPI] ✅ Chats encontrados:', {
            count: response.data?.length || 0
        });
        return {
            success: true,
        ...
        logger_1.default.error('[fetchChatsFromEvolutionAPI] ❌ Erro:', {
            message: error?.message,
            status: error?.response?.status,
            data: error?.response?.data
        });
 ...
 */
async function fetchMessagesFromChat(integrationsId, remoteJid, limit = 50) {
    try {
        const config = await getEvolutionAPICredentials(integrationsId);
        if (!config) {
            return { success: false, error: 'Credenciais não encontradas' };
        ...
        const instanceName = await createOrGetInstance(config);
        logger_1.default.log('[fetchMessagesFromChat] Buscando mensagens do chat...', {
            instanceName,
            remoteJid,
            limit
        });
        ...
        logger_1.default.log('[fetchMessagesFromChat] ✅ Mensagens encontradas:', {
            count: response.data?.length || 0
        });
        return {
            success: true,
        ...
        logger_1.default.error('[fetchMessagesFromChat] ❌ Erro:', {
            message: error?.message,
            status: error?.response?.status
        });
        return {
...
async function getAllUnreadMessages(integrationsId, agentId) {
    try {
        logger_1.default.log('[getAllUnreadMessages] Buscando todas as mensagens não lidas...', {
            integrations_id: integrationsId,
            agent_id: agentId
        });
        ...
        const { data, error } = await query;
        if (error) {
            logger_1.default.error('[getAllUnreadMessages] ❌ Erro ao buscar do banco:', {
                error: error.message
            });
            return [];
        }
        ...
        if (messagesFromDB.length === 0) {
            logger_1.default.warn('[getAllUnreadMessages] ⚠️ NENHUMA mensagem não lida encontrada no banco!', {
        ...
        const contactIds = [...new Set(messagesFromDB.map((msg) => msg.whatsapp_contact_id).filter(Boolean))];
        ...
        const messagesWithContacts = messagesFromDB.map((msg) => {
        ...
        logger_1.default.log('[getAllUnreadMessages] ✅ Últimas mensagens não lidas encontradas (uma por contato):', {
        ...
        logger_1.default.error('[getAllUnreadMessages] ❌ Erro:', {
            message: error?.message,
            stack: error?.stack
        });
        return [];
...
async function getContactNumberForSending(contactIdOrLid, integrationsId) {
    try {
        ...
        if (contactIdOrLid.includes('-') && contactIdOrLid.length === 36) {
            ...
            console.log('='.repeat(80) + '\n');
            if (error) {
                logger_1.default.error('[getContactNumberForSending] ❌ Erro ao buscar contato:', {
                    error: error.message
                });
        ...
        if (contactIdOrLid.endsWith('@lid')) {
            ...
            if (!contactResult.success || !contactResult.contact) {
                logger_1.default.log('[getContactNumberForSending] 🔍 LID não encontrado, tentando buscar pelo número...', {
                    lid: contactIdOrLid,
            ...
            if (contactResult.success && contactResult.contact) {
                ...
                if (contactResult.contact.phone_number && contactResult.contact.status === 'active') {
                    logger_1.default.log('[getContactNumberForSending] ✅ Contato encontrado (LID → número real):', {
                        lid: contactIdOrLid,
                ...
                logger_1.default.warn('[getContactNumberForSending] ⚠️ Contato encontrado mas sem número real:', {
                    lid: contactIdOrLid,
                ...
                logger_1.default.error('[getContactNumberForSending] ❌ LID e número não encontrados no banco:', {
                    lid: contactIdOrLid,
                    normalizedLid: normalizedLid
                });
                return {
        ...
        if (!contactIdOrLid.includes('@')) {
            ...
            if (contactResult.success && contactResult.contact) {
                ...
                logger_1.default.log('[getContactNumberForSending] ✅ Contato encontrado pelo número:', {
                ...
                logger_1.default.error('[getContactNumberForSending] ❌ Número não encontrado no banco:', {
                    number: normalizedNumber
                });
                return {
                    success: false,
        ...
        if (contactIdOrLid.endsWith('@s.whatsapp.net')) {
            ...
            if (!contactResult.success || !contactResult.contact) {
                logger_1.default.log('[getContactNumberForSending] 🔍 Número não encontrado, tentando buscar pelo LID...', {
            ...
            if (contactResult.success && contactResult.contact) {
                ...
                logger_1.default.log('[getContactNumberForSending] ✅ Contato encontrado (número real):', {
                    number: contactIdOrLid,
                    contactId: contactResult.contact.id,
                ...
                logger_1.default.error('[getContactNumberForSending] ❌ Número real e LID não encontrados no banco:', {
                    number: contactIdOrLid,
                    normalizedNumber: normalizedNumber
                });
                return {
        ...
        logger_1.default.error('[getContactNumberForSending] ❌ Formato não reconhecido:', {
            contactIdOrLid
        });
        return {
            success: false,
        ...
        logger_1.default.error('[getContactNumberForSending] ❌ Erro:', {
            message: error?.message
        });
        return {
            success: false,
 ...
 */
async function markMessagesAsRead(contactId, integrationsId) {
    try {
        const { error } = await supabase_1.supabase
            ...
            .eq('is_read', false);
        if (error) {
            logger_1.default.error('[markMessagesAsRead] ❌ Erro:', {
                error: error.message
            });
            return {
                success: false,
