...
exports.saveFallbackEvent = saveFallbackEvent;
 ...
 */
async function saveFallbackEvent(event) {
    try {
        ...
        if (!companyIdValue && event.user_email) {
            companyIdValue = await (0, company_helper_1.getCompanyIdByEmail)(event.user_email);
            if (companyIdValue) {
                logger_1.default.log(`[saveFallbackEvent] ✅ companies_id encontrado via user_email: ${companyIdValue}`);
            }
        }
        // Se não tiver user_id mas tiver user_email, buscar user_id
        if (!userIdValue && event.user_email) {
            userIdValue = await (0, company_helper_1.getUserIdByEmail)(event.user_email);
            if (userIdValue) {
                logger_1.default.log(`[saveFallbackEvent] ✅ user_id encontrado via user_email: ${userIdValue}`);
            }
        }
            ...
            try {
                ...
                if (!flowError && flowData) {
                    ...
                    if (flowData.companies_id) {
                        ...
                        logger_1.default.log(`[saveFallbackEvent] ✅ companies_id encontrado via workflow_id (campo direto): ${companyIdValue}`);
                    }
                    // Se não tiver companies_id mas tiver user_email, busca companies_id e user_id pelo email
                        ...
                        if (userCompanyData.userId && !userIdValue) {
                            ...
                            logger_1.default.log(`[saveFallbackEvent] ✅ user_id encontrado via workflow_id -> user_email: ${userIdValue}`);
                        }
                        if (userCompanyData.companyId) {
                            companyIdValue = userCompanyData.companyId;
                            logger_1.default.log(`[saveFallbackEvent] ✅ companies_id encontrado via workflow_id -> user_email: ${companyIdValue}`);
                        }
                    }
                }
            }
            catch (err) {
                logger_1.default.warn(`[saveFallbackEvent] Erro ao buscar companies_id via workflow_id: ${err.message}`);
            }
        }
        logger_1.default.log(`[saveFallbackEvent] Salvando evento:`, {
        ...
        const { data, error } = await supabase_1.supabase
            ...
            .single();
        if (error) {
            logger_1.default.error('[saveFallbackEvent] Erro ao salvar evento:', error);
            return { success: false, error: error.message };
        }
        logger_1.default.log(`[saveFallbackEvent] ✅ Evento de fallback salvo: ${event.event_type}`, {
            id: data.id,
            message: event.message,
        ...
        logger_1.default.error('[saveFallbackEvent] Erro:', err);
