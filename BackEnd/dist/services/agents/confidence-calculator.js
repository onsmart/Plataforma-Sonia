...
exports.DEFAULT_CONFIDENCE_APPROVAL_THRESHOLD = void 0;
exports.getConfidenceApprovalThreshold = getConfidenceApprovalThreshold;
exports.calculateConfidence = calculateConfidence;
/** Limiar padrão alinhado ao fluxo de aprovação no chatWithAgent */
exports.DEFAULT_CONFIDENCE_APPROVAL_THRESHOLD = 0.7;
/**
 * Permite ajustar o corte sem alterar código (ex.: 0.65 em produção).
 * Valores fora de (0,1) ignorados.
 */
function getConfidenceApprovalThreshold() {
    const raw = parseFloat(String(process.env.AGENT_CONFIDENCE_THRESHOLD ?? '').trim());
    if (!Number.isFinite(raw) || raw <= 0 || raw >= 1) {
        return exports.DEFAULT_CONFIDENCE_APPROVAL_THRESHOLD;
    }
    return raw;
}
/** Incerteza explícita do usuário — não confundir com "?" de pergunta direta */
const USER_UNCERTAINTY_RE = /\b(?:n[aã]o\s+sei|talvez|ser[aá]\s+que|acho\s+que|n[aã]o\s+tenho\s+certeza|n[aã]o\s+tenho\s+ideia|pode\s+ser|ou\s+seja)\b/i;
function isClearQuestion(message) {
...
function pickReason(current, next) {
    const rank = {
        insufficient_data: 4,
        ambiguous: 3,
        low_context: 2,
...
function calculateConfidence(parsedResponse, originalMessage, context, historyLength = 0, hasFileContext = false, sources) {
    const threshold = getConfidenceApprovalThreshold();
    const adjustments = [];
    let reason = 'high_match';
    const playgroundLike = context &&
    ...
    let confidence = 0.85;
    const apply = (label, delta, note, reasonHint) => {
    ...
    if (hasFileContext) {
        ...
        const isSubstantialResponse = responseLength > 40 &&
    ...
    if (interjectionPatterns.test(original)) {
        ...
        if (!hasFileContext)
            reason = pickReason(reason, 'low_context');
    }
    ...
    if (complexActions.includes(parsedResponse.action) && !context) {
        ...
        if (!hasFileContext)
            reason = pickReason(reason, 'insufficient_data');
    }
    ...
    console.log('🚦 Status:', requiresApproval ? '🛡️ BLOQUEADO (requer aprovação)' : '✅ APROVADO (envio automático)');
    console.log('═══════════════════════════════════════════════════════════');
    return {
