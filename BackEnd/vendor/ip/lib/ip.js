...
/** Buffer IPv6 ::1 loopback (::1), independent of textual encoding (GHSA-2p57-rm9w-gvfp). */
function ipv6BufferIsCanonicalLoopback(buf) {
...
ip.isPublic = function (addr) {
  ...
  // Endereços que o Node não considera válidos não podem ser tratados como "públicos".
  if (kind === 0) {
    return false;
  }
  try {
    if (kind === 6) {
      ...
      if (ipv6BufferIsCanonicalLoopback(buf)) {
        return false;
      }
    }
    if (kind === 4) {
      ...
      const canon = ip.fromLong(ipl);
      if (ip.isLoopback(canon) || ip.isPrivate(canon)) {
        return false;
      }
    }
  } catch {
