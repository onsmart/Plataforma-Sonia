...
set /p DESTINO="Digite o numero de destino (ex: 11943687794 ou 5511943687794): "
if "%DESTINO%"=="" (
...
REM Verificar se comeca com 55 usando uma abordagem mais simples
set DESTINO_TEMP=%DESTINO%
set DESTINO_TEMP=%DESTINO_TEMP:~0,2%
if not "%DESTINO_TEMP%"=="55" (
    echo.
...
curl -X POST "%EVOLUTION_API_URL%/message/sendText/%INSTANCE_NAME%" ^
  -H "apikey: %API_KEY%" ^
  -H "Content-Type: application/json" ^
  -d "{\"number\": \"%DESTINO%\", \"text\": \"%MENSAGEM%\"}"
