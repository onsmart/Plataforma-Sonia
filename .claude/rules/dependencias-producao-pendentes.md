...
| `[ ]` | **Observabilidade — variáveis permanentes no `.env`** | `METRICS_BEARER_TOKEN` e `SENTRY_DSN` adicionados via shell export (`pm2 restart --update-env`). Se o servidor reiniciar, o backend sobe sem essas variáveis. Adicionar permanentemente em `~/plataform-backend/BackEnd/.env`. |
