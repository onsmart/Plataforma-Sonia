...
| `[x]` | PM2 boot automático no servidor | Resolvido 2026-06-09: `pm2 startup` + `pm2 save` executados. `pm2-servidoronsmart.service` habilitado no systemd. |
| `[x]` | **Observabilidade — variáveis permanentes no `.env`** | Resolvido 2026-06-09: `METRICS_BEARER_TOKEN` e `SENTRY_DSN` confirmados permanentemente em `~/plataform-backend/BackEnd/.env`. |
| `[ ]` | **Observabilidade — Docker no auto-start** | Containers de observabilidade (Prometheus, Grafana, Loki, Promtail, Node Exporter) não sobem automaticamente após reboot. Resolver: `sudo systemctl enable docker` + criar serviço systemd para `docker compose up` em `~/observabilidade/`. |
| `[ ]` | **Observabilidade — containers no auto-start após reboot** | Mesmo com Docker no auto-start, os containers precisam ser levantados manualmente. Criar `/etc/systemd/system/observabilidade.service` com `ExecStart=docker compose -f /home/servidoronsmart/observabilidade/docker-compose.yml up` e habilitar com `systemctl enable observabilidade`. |
